...
Run the script as shown, keeping `${CLAUDE_SKILL_DIR}` so the shell resolves it against this skill's directory.

```sh
python3 ${CLAUDE_SKILL_DIR}/scripts/openalex.py check
```
...
```sh
python3 ${CLAUDE_SKILL_DIR}/scripts/openalex.py search "CRISPR off-target effects" --year '>2020' --oa --limit 50
python3 ${CLAUDE_SKILL_DIR}/scripts/openalex.py search "$(cat aim.txt)" --semantic --year '>2015'
python3 ${CLAUDE_SKILL_DIR}/scripts/openalex.py search "malaria vector control" --sort cited_by_count:desc --limit 10
```
...
```sh
python3 ${CLAUDE_SKILL_DIR}/scripts/openalex.py search "" --filter authorships.author.id:A5023888391 --year 2020-2024
```
...
```sh
python3 ${CLAUDE_SKILL_DIR}/scripts/openalex.py resolve author "Jennifer Doudna"
python3 ${CLAUDE_SKILL_DIR}/scripts/openalex.py resolve institution "Gates Foundation"
```
...
```sh
python3 ${CLAUDE_SKILL_DIR}/scripts/openalex.py get A5023888391
python3 ${CLAUDE_SKILL_DIR}/scripts/openalex.py get issn:2167-8359
```
...
```sh
python3 ${CLAUDE_SKILL_DIR}/scripts/openalex.py group publication_year --query "doxycycline prophylaxis"
python3 ${CLAUDE_SKILL_DIR}/scripts/openalex.py group topics.id --filter authorships.institutions.id:I4210088555
python3 ${CLAUDE_SKILL_DIR}/scripts/openalex.py group open_access.oa_status --year 2023
```
...
```sh
python3 ${CLAUDE_SKILL_DIR}/scripts/openalex.py batch --file docs/sources/refs.txt
```
...
```sh
python3 ${CLAUDE_SKILL_DIR}/scripts/openalex.py get 10.1038/s41586-021-03819-2
python3 ${CLAUDE_SKILL_DIR}/scripts/openalex.py cited-by 10.1038/s41586-021-03819-2 --limit 25
```
