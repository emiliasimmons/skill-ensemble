...
Write the resolved items to `docs/raw/zotero-<selector>.bib`, where `<selector>` is the collection or tag name slugified, or `recent`. Re-running a selector overwrites the file. `bib_status.json` persists across rewrites and keys on DOI, so cataloging status survives and the overwrite shows what the selector picked up since last time.
...
```sh
uv run scripts/extract_pdf.py <attachment-path> -o docs/raw/<bibtex_key>/
```
...
```sh
uv run scripts/process_bib.py mark docs/raw/zotero-<selector>.bib extracted <key>
uv run scripts/process_bib.py mark docs/raw/zotero-<selector>.bib extracted <key> --via zotero
```

When neither returns text, the item's `abstractNote` is what remains: take the abstract-only route in SKILL.md.
